...
def add(request):
    ...
    # add story image
    storyPic = request.FILES.get('storyPic', False)
    
    story = Tmas(subject=subject, date=date, story=story, storyID=storyID, user=user, location=location,
                 community=community, status=status, storyPic=storyPic)
    story.save()
...
def editReplace(request, storyID):
    ...
    # add story image
    storyPicNow = request.FILES.get('storyPic', False)
    
    if storyPicNow == False:
        storyPic = storyPicPrev
    else:
        storyPic = storyPicNow
